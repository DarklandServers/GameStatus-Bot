...
try {
    ...
    for(var i = 0; i < runEnabled.length; i++) { 
            ...
            if (apiType === 1) {
                ...
                request({ url: apiUrl, headers: { json: true}, timeout: 10000 }, function (err, res, body) {
                    if (!err && res.statusCode == 200) {
                        ...
                        if (is_online == 'online') {
                            ...
                            //RETURN OFFLINE
                            return client.user.setPresence({ activities: [{ name: 'Offline', type: ActivityType.Custom}], status: 'idle'});
                        } 
                    }
                });
            } else if (apiType === 2) {
                if (!serverIp || !serverPort) {
                    ...
                    Gamedig.query({
                        ...
                        status = showMap ? `${players}/${maxplayers} ${configData.statusSpacer} ${mapName}` : `${players}/${maxplayers}`;
                        //RETURN ONLINE
                        return client.user.setPresence({ activities: [{ name: `${configData.statusPrefix} ${status}`, type: ActivityType.Custom}], status: 'online'});
                    }).catch(() => {
                        //RETURN OFFLINE
        ...
        const { botToken, apiType, apiUrl, serverIp, serverPort, queueMessage, showMap, mapPrefix, gameType, debug } = config;
